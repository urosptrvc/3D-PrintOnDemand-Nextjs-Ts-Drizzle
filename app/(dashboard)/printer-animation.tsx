...
function PrinterModel() {
    ...
    const [extruderPosition, setExtruderPosition] = useState({ x: 0, y: 0.5, z: 0 });
    ...
    useFrame((state, delta) => {
      if (
        ...
        extruderHeadRef.current.rotation.z = Math.sin(state.clock.elapsedTime * 10) * 0.01;
      }
    });
  ...
  return (
    <group ref={printerRef}>
      ...
      <mesh position={[-1.2, 0.5, 0]} castShadow>
        ...
        <meshStandardMaterial color="#444" transparent opacity={0.15} metalness={0.8} roughness={0.1} />
      </mesh>
      <mesh position={[1.2, 0.5, 0]} castShadow>
        <boxGeometry args={[0.05, 1.9, 2.4]} />
        <meshStandardMaterial color="#444" transparent opacity={0.15} metalness={0.8} roughness={0.1} />
      ...
      <mesh position={[0, -0.3, 0]} ref={printBedRef} receiveShadow>
        ...
        <meshStandardMaterial color="#999" roughness={0.2} metalness={0.6} envMapIntensity={1.2} />
      </mesh>
      
      {/* Print bed grid pattern */}
      <mesh position={[0, -0.24, 0]} rotation={[Math.PI/2, 0, 0]} receiveShadow>
        <planeGeometry args={[1.9, 1.9]} />
        <meshStandardMaterial color="#aaa" wireframe={true} emissive="#333" emissiveIntensity={0.1} />
      </mesh>
      
      {/* Extruder Assembly with X-carriage */}
      <group ref={extruderRef} position={[extruderPosition.x, 1.2, extruderPosition.z]}>
        {/* Extruder Carriage */}
        <mesh castShadow>
        ...
        <group ref={extruderHeadRef}>
          ...
          <group position={[0, -0.28, 0]}>
            ...
            <mesh castShadow>
              ...
              <meshStandardMaterial color="#444" metalness={0.95} roughness={0.1} envMapIntensity={1.5} />
            </mesh>
            {/* Nozzle tip */}
            <mesh position={[0, -0.05, 0]} castShadow>
              <coneGeometry args={[0.025, 0.04, 16, 1]} />
              <meshStandardMaterial color="#222" metalness={1} roughness={0.05} envMapIntensity={2} />
            </mesh>
          </group>
      ...
      >
        ...
        <meshStandardMaterial color="#f5822d" /> {/* Orange color matching the site theme */}
      </mesh>
    </group>
  );
}
...
export function PrinterAnimation() {
  return (
    <div className="w-full h-full">
      <Canvas 
        shadows 
        camera={{ position: [4, 0.5, -2], fov: 70 }}
